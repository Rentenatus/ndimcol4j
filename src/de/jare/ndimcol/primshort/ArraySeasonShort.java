...
import java.util.*;
// ShortConsumer is unfortunately not available in java.util.function
import java.util.function.IntConsumer;
// ShortPredicate is unfortunately not available in java.util.function
 ...
 */
    // #### This code has been generated. Please do not make any changes here.
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
public class ArraySeasonShort implements ArrayMovieShort {

    ScreenplayShort screenplay;
    de.jare.ndimcol.ref.ArrayTape<ArrayMovieShort> data;
    int size;
    int maxEpisodeSize;
    int midEpisodeSize;
    int minEpisodeGlue;
    ...
    private ArrayMovieShort lastEpisode = null;
    private IterSeasonWalkerShort softWalker;

    ArraySeasonShort(de.jare.ndimcol.ref.ArrayTape<ArrayMovieShort> data, int size) {
        screenplay = Screenplay2dShort.INSTANCE;
        this.data = data;
        this.size = size;
        updateCounter = 0;
        recalculateScope();
    ...
    // #### This code has been generated. Please do not make any changes here.
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    public ArraySeasonShort(final ScreenplayShort screenplay) {
    ...
    public boolean add(short element) {
        ...
        final ArrayMovieShort episode = data.last();
        if (episode.size() >= midEpisodeSize && episode.pageSpaceLeft() <= 3) {
            final ArrayMovieShort nextFree = buildInnerMovie(this.size);
    ...
    public boolean addFirstFree(short element) {
        ...
        if (episodeIndex == -1) {
            ...
            ArrayMovieShort episode = data.get(episodeIndex);
    ...
    public boolean addAt(int index, short element) {
        ...
        for (int i = 1; i < dataSize; i++) {
            final ArrayMovieShort episode = data.get(i);
            int episodeSize = episode.size();
...
//        return false;
    ...
    public boolean addAll(Collection<? extends Short> col) {
        ...
        final ArrayMovieShort episode = data.get(data.size() - 1);
    ...
    public boolean addAll(short[] arr) {
        ...
        final ArrayTapeShort episode = new ArrayTapeShort(arr.length);
        episode.addAll(arr);
    ...
    public boolean addMovie(ArrayMovieShort movie) {
        final ArrayTapeShort episode = new ArrayTapeShort(movie.size());
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public boolean glueMovie(ArrayMovieShort episode) {
        if (episode.isEmpty()) {
            return false;
        }
    ...
    @Override
    public void assimilateInto(de.jare.ndimcol.ref.ArrayTape<ArrayMovieShort> othersData) {
    ...
    public boolean equals(Object ob) {
        if (!(ob instanceof ArrayMovieShort)) {
            if (!(ob instanceof Collection<?>)) {
                return false;
            }
            return equalsCollection((Collection<?>) ob);
        }
        ArrayMovieShort movie = (ArrayMovieShort) ob;
        if (size() != movie.size()) {
            return false;
        }
        IteratorWalkerShort mWalker = movie.softWalker();
        IteratorWalkerShort walker = softWalker();
    ...
    public boolean equals(short a, Object b) {
        return _equals(a, b);
    }
    @Override
    public int hashCode() {
    ...
    public boolean remove(short element) {
        IteratorWalkerShort walker = getWalkerAtElement(element);
        if (walker == null) {
            return false;
        }
    ...
    public void splitOrGlue() {
        ...
        for (int i = 0; i < data.size(); i++) {
            ArrayMovieShort episode = data.get(i);
            ...
            if (episodeSize == 0) {
                ...
                lastEpisode = null;
                ArrayMovieShort newEpisode = episode.splitInHalf();
                if (newEpisode == null) {
                    continue;
                }
                ...
                data.removeAt(i);
                i--; // Move back to recheck the merged episode
                final ArrayMovieShort prevEpisode = data.get(i);
    ...
    protected void recalculateScope() {
        ...
        midEpisodeSize = maxEpisodeSize - ArrayTapeShort.DEFAULT_PAGE;
    }
    ...
    // #### This code has been generated. Please do not make any changes here.
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    protected IteratorWalkerShort getWalkerAtIndex(int index) {
        if (lastEpisode != null
                && lastAccumulatedSize <= index
                && index < lastAccumulatedSize + lastEpisode.size()) {
            return new IterCoverWalkerShort(this, lastEpisode.leafWalker(index - lastAccumulatedSize));
        }
        int accumulatedSize = 0;
        for (int i = 0; i < data.size(); i++) {
            final ArrayMovieShort episode = data.get(i);
            ...
            if (index < accumulatedSize + episodeSize) {
                lastAccumulatedSize = accumulatedSize;
                lastEpisode = episode;
                return new IterCoverWalkerShort(this, episode.leafWalker(index - accumulatedSize));
            }
            accumulatedSize += episodeSize;
        }
        throw new IndexOutOfBoundsException("Index: " + index + ", Size: " + size + ".");
    ...
    // #### This code has been generated. Please do not make any changes here.
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    public IteratorWalkerShort getWalkerAtElement(final short element) {
        for (int i = 0; i < data.size(); i++) {
            final ArrayMovieShort episode = data.get(i);
            final int episodeIndex = episode.indexOf(element);
            if (episodeIndex >= 0) {
                return new IterCoverWalkerShort(this, episode.leafWalker(episodeIndex));
...
//        return filterFirst(element::equals);
    ...
    public int indexOf(short element) {
        final int dataSize = data.size();
        if (dataSize == 1) {
            final ArrayMovieShort episode = data.first();
    ...
    // #### This code has been generated. Please do not make any changes here.
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    public int getOffset(ArrayMovieShort relatedMovie) {
        int offset = 0;
        for (int i = 0; i < data.size(); i++) {
            ArrayMovieShort episode = data.get(i);
    ...
    public boolean contains(short element) {
        for (ArrayMovieShort episode : data) {
    ...
    public IteratorShort iterator() {
        final IterSeasonWalkerShort walker = new IterSeasonWalkerShort(this);
    ...
    @Override
    public short[] toArray(short[] arr) {
        if (arr.length < size) {
            arr=new short[size];
        }
        copyToArray(arr, 0);
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public void copyToArray(short[] arr, int offset) {
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public ArrayMovieShort subMovie(int fromIndex, int toIndex) {
        ...
        ArraySeasonShort subMovie = emptyMovie(0);
        if (fromIndex == toIndex) {
            return subMovie;
        }
        int accumulatedSize = 0;
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public ArraySeasonShort emptyMovie(int initialCapacityOrZero) {
        return new ArraySeasonShort(screenplay);
    }
    ...
    public boolean containsAll(Collection<?> col) {
        if (col == null) {
         ...
         for (Object element : col) {
             if ( !(element instanceof Short) || !contains((short)element) ) {
                return false;
            }
        }
        return true;
    ...
    void replaced(int index, short ret, short element) {
        //NoOp
    }
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public ArrayMovieShort splitInHalf() {
        if (data.isEmpty()) {
            return null;
        }
        lastEpisode = null;
        lastAccumulatedSize = 0;
        this.updateCounter++;
        ArraySeasonShort ret = emptyMovie((size >> 1) + screenplay.getDefaultSize());
        ret.screenplay = screenplay;
        if (data.size() == 1) {
            ArrayMovieShort other = data.get(0).splitInHalf();
            updateSize();
            if (other == null) {
                return null;
            }
            ...
            de.jare.ndimcol.ref.ArrayTape<ArrayMovieShort> other = data.splitInHalf();
            updateSize();
            if (other == null) {
                return null;
            }
    ...
    @Override
    public IteratorWalkerShort softWalker() {
        if (softWalker != null) {
            return softWalker.goFirst();
        }
        softWalker = new IterSeasonWalkerShort(this);
        return softWalker;
    }

    @Override
    public IteratorWalkerShort softWalkerBackwards() {
        if (softWalker != null) {
    ...
    @Override
    public IteratorWalkerShort leafWalker(int atIndex) {
        if (softWalker != null) {
    ...
    @Override
    public IteratorWalkerShort filterFirst(ShortPredicate predicate) {
        int startupdateCounter = updateCounter;
        for (int i = 0; i < data.size(); i++) {
            IteratorWalkerShort walker = data.get(i).filterFirst(predicate);
            if (walker != null) {
                if (startupdateCounter != updateCounter) {
                    throw new ConcurrentModificationException("ArraySeasson was modified during iteration.");
                }
                return new IterCoverWalkerShort(this, walker);
            }
        }
        return null;
    }
    ...
    @Override
    public IteratorWalkerShort filterLast(ShortPredicate predicate) {
        int startupdateCounter = updateCounter;
        for (int i = data.size() - 1; i >= 0; i--) {
            IteratorWalkerShort walker = data.get(i).filterLast(predicate);
            if (walker != null) {
                if (startupdateCounter != updateCounter) {
                    throw new ConcurrentModificationException("ArraySeasson was modified during iteration.");
                }
    ...
    @Override
    public ArrayMovieShort filterAll(ShortPredicate predicate) {
        ArraySeasonShort ret = emptyMovie(data.size() << 3);
        de.jare.ndimcol.ref.IterTapeWalker<ArrayMovieShort> walker = data.walker();
        while (walker.hasNext()) {
            ArrayMovieShort elements = walker.next();
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public de.jare.ndimcol.primint.ArrayMovieInt filterAllIndexes(ShortPredicate predicate) {
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public void forEach(ShortPredicate predicate, ShortConsumer thenAction) {
        de.jare.ndimcol.ref.IterTapeWalker<ArrayMovieShort> walker = data.walker();
        while (walker.hasNext()) {
            ArrayMovieShort elements = walker.next();
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public void forEach(ShortPredicate predicate, ShortConsumer thenActionOrNull,
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @Override
    public void forEach(ShortPredicate predicate,
            ShortConsumer thenAction, ShortConsumer elseAction) {
        de.jare.ndimcol.ref.IterTapeWalker<ArrayMovieShort> walker = data.walker();
        while (walker.hasNext()) {
            ArrayMovieShort elements = walker.next();
    ...
    // #### Modify package 'de.jare.ndimcol.ref' and use 'GeneratePrimitiveJavaFiles'
    @SuppressWarnings("unchecked")
    public ArrayMovieShort filterParallel(ShortPredicate predicate) {
        Thread[] threads = new Thread[data.size()];
        Runnable[] tasks = new Runnable[data.size()];
        for (int i = 0; i < data.size(); i++) {
            tasks[i] = new PredicateAllRunnableShort(predicate, data.get(i));
        ...
        for (int i = 0; i < data.size(); i++) {
            try {
                threads[i].join();
                ArrayMovieShort elements = ((PredicateAllRunnableShort) tasks[i]).getElements();
                if (elements.hasRecord()) {
                    ret.data.add(elements);
                }
