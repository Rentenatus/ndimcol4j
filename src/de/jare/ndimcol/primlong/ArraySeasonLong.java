...
public class ArraySeasonLong implements ArrayMovieLong {
    ...
    de.jare.ndimcol.ref.ArrayTape<ArrayMovieLong> data;
    int size;
    int maxEpisodeSize;
    int midEpisodeSize;
    int minEpisodeGlue;
    ...
    public boolean add(long element) {
        if (data.isEmpty()) {
            final ArrayMovieLong first = buildInnerMovie(0);
    ...
    public boolean addFirstFree(long element) {
        ...
        if (episodeIndex == -1) {
            ...
            ArrayMovieLong episode = data.get(episodeIndex);
    ...
    public boolean addAt(int index, long element) {
        ...
        if (index == size) {
            ...
            final ArrayMovieLong last = data.get(dataSize - 1);
            lastEpisode = null;
            if (!last.add(element)) {
                return false;
            }
...
//        return false;
    ...
    public boolean addAll(long[] arr) {
        ...
        final ArrayTapeLong episode = new ArrayTapeLong(arr.length);
        episode.addAll(arr);
    ...
    public boolean equals(Object ob) {
        ...
        IteratorWalkerLong walker = softWalker();
...
//        return removedElement;
    ...
    public void splitOrGlue() {
        ...
        for (int i = 0; i < data.size(); i++) {
            ArrayMovieLong episode = data.get(i);
            ...
            if (episodeSize == 0) {
                ...
                lastEpisode = null;
                ArrayMovieLong newEpisode = episode.splitInHalf();
                if (newEpisode == null) {
                    continue;
                }
                ...
                data.removeAt(i);
                i--; // Move back to recheck the merged episode
                final ArrayMovieLong prevEpisode = data.get(i);
    ...
    protected void recalculateScope() {
        ...
        midEpisodeSize = maxEpisodeSize - ArrayTapeLong.DEFAULT_PAGE;
    }
    ...
    protected IteratorWalkerLong getWalkerAtIndex(int index) {
        if (lastEpisode != null
                && lastAccumulatedSize <= index
                && index < lastAccumulatedSize + lastEpisode.size()) {
            return new IterCoverWalkerLong(this, lastEpisode.leafWalker(index - lastAccumulatedSize));
        }
        int accumulatedSize = 0;
        for (int i = 0; i < data.size(); i++) {
            final ArrayMovieLong episode = data.get(i);
            ...
            if (index < accumulatedSize + episodeSize) {
                lastAccumulatedSize = accumulatedSize;
                lastEpisode = episode;
                return new IterCoverWalkerLong(this, episode.leafWalker(index - accumulatedSize));
            }
            accumulatedSize += episodeSize;
        }
        throw new IndexOutOfBoundsException("Index: " + index + ", Size: " + size + ".");
    ...
    public int indexOf(long element) {
        final int dataSize = data.size();
        if (dataSize == 1) {
            final ArrayMovieLong episode = data.first();
    ...
    public ArrayMovieLong subMovie(int fromIndex, int toIndex) {
        ...
        ArraySeasonLong subMovie = emptyMovie(0);
        if (fromIndex == toIndex) {
            return subMovie;
        }
        int accumulatedSize = 0;
    ...
    public boolean containsAll(Collection<?> col) {
        if (col == null) {
         ...
         for (Object element : col) {
             if ( !(element instanceof Long) || !contains((long)element) ) {
                return false;
            }
        }
        return true;
    ...
    void replaced(int index, long ret, long element) {
        //NoOp
    }
    ...
    public ArrayMovieLong splitInHalf() {
        ...
        lastAccumulatedSize = 0;
        this.updateCounter++;
        ArraySeasonLong ret = emptyMovie((size >> 1) + screenplay.getDefaultSize());
        ret.screenplay = screenplay;
        if (data.size() == 1) {
            ArrayMovieLong other = data.get(0).splitInHalf();
            updateSize();
            if (other == null) {
                return null;
            }
            ...
            de.jare.ndimcol.ref.ArrayTape<ArrayMovieLong> other = data.splitInHalf();
            updateSize();
            if (other == null) {
                return null;
            }
    ...
    @Override
    public IteratorWalkerLong softWalker() {
        if (softWalker != null) {
    ...
    public ArrayMovieLong filterParallel(LongPredicate predicate) {
        ...
        for (int i = 0; i < data.size(); i++) {
            tasks[i] = new PredicateAllRunnableLong(predicate, data.get(i));
        ...
        for (int i = 0; i < data.size(); i++) {
            try {
                threads[i].join();
                ArrayMovieLong elements = ((PredicateAllRunnableLong) tasks[i]).getElements();
                if (elements.hasRecord()) {
                    ret.data.add(elements);
                }
